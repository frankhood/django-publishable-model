...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-publishable-model runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-publishable-model.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ publishable_model")
...
def lint(c):
    ...
    """
    c.run("flake8 django-publishable-model tests")
...
def release(c, bumpsize=''):
    ...
    import django_publishable_model
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_publishable_model.__version__))
    c.run("git push --tags")
